...
export default function PhoneChatScreen() {
  ...
  const [requestsContainerWidth, setRequestsContainerWidth] = useState<number>(0);
  ...
  async function handleRequests(user: string, accepted: boolean) {
    ...
    const requestToRemoveIndex = currentToAddRequestsList.outgoing.indexOf(uData.username);
  ...
  async function deleteRequest(user: string) {
    ...
    let currentToAddRequestsList = userToRemove.data[0].friendRequests;
    console.log(currentToAddRequestsList);
    const requestToRemoveIndex = currentToAddRequestsList.recieved.indexOf(uData.username);
        ...
        >
          ...
          <View style={{ width: "70%", marginHorizontal: "auto" }}>
            ...
            <View style={{ flex: 1, overflow: "hidden", flexDirection: "row" }}
              ...
              <Animated.View style={[{ flexDirection: "row", width: "100%" }, animatedXStyle]}>
                  ...
                  <FlatList
                    ...
                    renderItem={({ item }) => (
                      <View style={{ padding: 10 }}>
                        <Text style={styles.text}>You sent a friend request to {item}.</Text>
                        <View>
                          <Pressable style={styles.navButton}
                ...
                <View style={{ width: "100%" }}>
                  <FlatList
                    ...
                    renderItem={({ item }) => (
                      <View style={{ padding: 10 }}>
                        <Text style={styles.text}>{item} sent you a friend request!</Text>
                        <View style={{ flexDirection: "row" }}>
                          <Pressable style={styles.navButton}
                            onPress={() => { handleRequests(item, true) }}
                          >
                            <Feather name="check" size={16} color="green" />
                          </Pressable>
                          <Pressable style={styles.navButton}
                            onPress={() => { handleRequests(item, false) }}
                          >
                            <Feather name="x" size={16} color="red" />
                          </Pressable>
                        </View>
                    ...
                    contentContainerStyle={
                      uData.friendRequests.recieved.length === 0 ? { flexGrow: 1 } : null
                    }
                    ListEmptyComponent={
                      <View
                        style={{
