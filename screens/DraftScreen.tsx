...
import { View, StyleSheet, SafeAreaView, TextInput, Text, ScrollView, Pressable, useWindowDimensions, Image, FlatList } from "react-native";
...
import { positionAbbriviations, regionAbbr, positionIcons } from "@/assets/positionIcons";
...
type AuthScreenNavigationProp = StackNavigationProp<RootStackParamList, "Draft">;
...
export default function DraftScreen() {
     ...
     function getPlayerNames() {
          ...
          const orderedUsersBySeed = clData.teamsPlaying.sort((a: any, b: any) => a.seed - b.seed);
          ...
          for (let i = 0; i < 9; i++) {
               ...
               for (let j = 0; j < clData.numPlayers; j++) {
                    if (takenPlayerData[count]) {
                         ...
                         rowArr.push(
                              <View style={
                                   [{ width: 128, height: 64, padding: 1 }]
                              }
                                   key={i + j}>
                                   <View style={{ flex: 1, padding: 2, borderRadius: 5, backgroundColor: colors[currentPlayer.position as keyof typeof colors] }}>
                                        <View style={
                                             {
                                                  flexDirection: "row",
                                             ...
                                             <Text style={[styles.subtext, { color: "rgb(118 118 118)", fontSize: 10 }]}>
                                                  {positionAbbriviations[currentPlayer.position as keyof typeof positionAbbriviations]} {"-"} {currentPlayer.teamAbbr} {"-"} {regionAbbr[currentPlayer.region as keyof typeof regionAbbr]}
                                             </Text>
                                             <Text style={styles.subtext}>
                                                  {i + 1}.{j + 1}
                                             </Text>
                                        </View>
                                        <Text style={{ fontSize: 16, color: "black", fontFamily: "tex" }}>{currentPlayer.player}</Text>
                                   </View>
                              </View>
                         )
                    } else {
                         rowArr.push(
                              <View style={
                                   ...
                                   key={i + j}>
                                   <View style={{ flex: 1, backgroundColor: colors.subtext, borderRadius: 5, padding: 2 }}>
                                        <View style={
                                             {
                                                  flexDirection: "row",
                                                  justifyContent: "space-between"
               ...
               arr.push(<View key={i} style={i % 2 === 0 ? { flexDirection: "row" } : { flexDirection: "row-reverse" }}>{rowArr}</View>);
          }
          setDraftedPlayersGrid(arr);
     }

     async function getTakenPlayersData(data: any) {
          const response = await database.from("players").select('*').in("player", data);
          if (response.data) {
               const sortedArray = response.data.sort((a: any, b: any) => takenPlayers?.takenPlayers.indexOf(a.player) - takenPlayers?.takenPlayers.indexOf(b.player));
     ...
     useEffect(() => {
          ...
          async function getTakenPlayers() {
               const response = await database.from("leagues").select("*").eq("leagueID", currentLeagueID);
               if (response.data) {
                    setTakenPlayers(
                         {
     ...
     useEffect(() => {
          setUserDataForCurrentLeague(clData.teamsPlaying.find((player: { playerID: any; }) => player.playerID === userData.id));
     }, [clData])
     ...
     async function increaseCurrentTurn() {
          ...
          const sigma = await database.from("leagues").update({ currentTurn: turn, forward: f }).eq("leagueID", currentLeagueID);
     }
          ...
          if (takenPlayers?.takenPlayers) {
               ...
               if (takenPlayers.takenPlayers.length === clData.numPlayers * 9) {
                    ...
                    async function isDraftingFalse() {
                         await database.from("leagues").update({ isDrafting: false, isDrafted: true }).eq("leagueID", currentLeagueID);
                    }
          ...
          >
               <SafeAreaView style={styles.container}>
                    <View style={{ flexDirection: "row", justifyContent: "space-between", width: "100%", alignItems: "center" }}>
                         ...
                         <Text style={{ color: "white", fontFamily: "tex", fontSize: 16, textAlign: "center" }}>{clData["league-name"]}</Text>
               ...
               <Animated.View style={[styles.modal, animatedStyle]}>
                    ...
                    >
                         <Pressable onPress={() => { increaseCurrentTurn() }}>INCREASE TURN!!</Pressable>
                         <Pressable
                              style={{
                                   padding: 20,
                                   width: "100%"
                                   ...
                                   renderItem={({ item, index }) => (
                                        ...
                                        >
                                             ...
                                             {currentTurn === userDataForCL?.seed && clData.isDrafting ?
                                                  <Pressable
                                                       style={{ marginRight: 10, paddingHorizontal: 10, paddingVertical: 5, backgroundColor: "#5A3EA1", borderRadius: 15 }}
                                                       onPress={() => {
                                                            draftPlayer(item);
                                                       }}
                                                  >
                                                       <Text style={{ fontFamily: "tex", color: "white" }}>DRAFT</Text>
                                                  </Pressable> :
                                                  <View
                                                       style={{ marginRight: 10, paddingHorizontal: 10, paddingVertical: 5, backgroundColor: colors.subtext, borderRadius: 15 }}
                                                  >
                                                       <Text style={{ fontFamily: "tex", color: "white" }}>DRAFT</Text>
                                                  </View>
                                             }
